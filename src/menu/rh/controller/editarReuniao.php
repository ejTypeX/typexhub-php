<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        header('Location: ../../editarReuniao.php');
        exit;
    }

    try{
        // usar null coalescing para evitar avisos
        $id = isset($_POST['reuniao_id']) ? (int) $_POST['reuniao_id'] : 0;
        ...
        $success = updateReuniao($pdo, $id, $reuniao);

        if ($success) {
            $_SESSION['flash_success'] = 'Reunião atualizada com sucesso.';
            $response['success'] = true;
            $response['message'] = 'Reunião atualizada com sucesso.';
            header('Location: ../editarReuniao.php?id=' . $id . '&success=' . urlencode($response['message']));
            exit;
        } else {
        ...
        header('Location: ../../editarReuniao.php?error=' . urlencode($e->getMessage()));
        exit;
    }
